...
# 
# __回答:__ human_files能够检测到人脸的图像占比100%，dog_files能够检测到人脸的图像占比11%
# 
...
human_files_face = len([face_detector(image) for image in human_files_short 
                        if face_detector(image)==True ])

dog_files_face = len([face_detector(image) for image in dog_files_short 
                        if face_detector(image)==True ])

print("Percentage of face(s) detected in huamn images: {:.2f}%".format(human_files_face))
print("Percentage of face(s) detected in dog images: {:.2f}%".format(dog_files_face))
...
# 
# __回答:__ 用户提供含有清晰面部特征的人脸图像这一要求对于人脸识别Face Recognition合理，但对于单纯人脸检测Face Detection不太合理，算法可能不能检测训练数据以外的情形，诸如光线变化、模糊等。可以使用图像增强的方法，将各种场景下的图像添加至训练集中让模型学习。
# 
# 
...
# 
# __回答:__ human_files检测到狗狗的图像占比0%，dog_files检测到狗狗的图像占比100%

# In[11]:


### TODO: 测试dog_detector函数在human_files_short和dog_files_short的表现
human_files_dog = len([dog_detector(image) for image in human_files_short 
                        if dog_detector(image)==True ])

dog_files_dog = len([dog_detector(image) for image in dog_files_short 
                        if dog_detector(image)==True ])

print("Percentage of dog(s) detected in huamn images: {:.2f}%".format(human_files_dog))
print("Percentage of dog(s) detected in dog images: {:.2f}%".format(dog_files_dog))
...
# __回答:__ 
# ### CNN部分
# 使用4个2x2小窗口卷积层学习图像特征，相邻卷积层间穿插最大池化层，在降低维度的同时保留图像主要特征，最后扁平化图像传递给全连接层。
#     
#     窗口大小2x2，过滤器数量16的卷积层
#     最大池化层
#     窗口大小2x2，过滤器数量32的卷积层
...
# 
# ### MLP部分
# 由2个全连接层组成，第一个全连接层包含266个节点，使用ReLU激活函数。第二个全连接层作为输出，有133个节点，使用多分类softmax作为激活函数，输出结果。
...
# 
# 
# __回答:__ 我们的训练集相对还是比较小的，预测的是狗的品种，ImageNet预测的是物体种类，相差比较大。为防止训练过程中过拟合，保留ResNet50的大部分结构，冻结参数，在模型末尾添加全局平均池化层降低维度，新添加一个全连接层作为输出层，节点数为分类品种总数133，使用多分类softmax作为激活函数。初始化这一层的参数，进行训练，选定最佳参数作为最终模型。
# 
...
def detector(img_path):
    ...
    breed = ''.join(re.findall('\w', ResNet50_predict_breed(img_path))[5:])
    
    if dog_detected:
        print('This is a dog, its breed is {}.'.format(breed))
    elif human_detected:
        print('This is a human, he/she looks like {}.'.format(breed))
    else:
        print("Sorry, can't recognize this image :(")
...
# 
# __回答:__ 
#     1. 比我想的要好诶，狗狗品种都预测对了，也能正确分辨狗和人类
#     2. a)图像增强
#        b)扩充数据，增加一些不同场景下的图片，例如不同光线，不同角度等等
#        c)继续调优参数
...
## 自由地使用所需的代码单元数吧
file_name_list = glob("test_images/*.jpg") 


# In[34]:


detector(file_name_list[0])


# In[35]:


detector(file_name_list[1])


# In[36]:


detector(file_name_list[2])


# In[37]:


detector(file_name_list[3])


# In[38]:


detector(file_name_list[4])


# In[39]:


detector(file_name_list[5])
